...
export function ConfigForm({ initial, chainId, onSubmit }: ConfigFormProps) {
  ...
  const removeOwner = (id: string) => {
    setOwners((rows) => {
      ...
      // The row above, which always exists: the first row has no remove button, so nothing can be
      // removed from position 0.
      focusRow.current = rows[index - 1].id
            ...
            return (
              <div key={owner.id} className="flex flex-col gap-1">
                <div className="flex items-end gap-2">
                  ...
                  {/* No remove on the first row: there is always at least one owner, and
                      validateMineConfig rejects an empty list outright. */}
                  {index > 0 && (
                    <Button
                      type="button"
                      variant="ghost"
